    ...
    for (i = 0; i < 5; i++) {
        if (belanja[i] == item[0].nama) {
            if (jumlah[i] > item[0].jumlah) {
                $('#warning2').html('<span> Maaf. jumlah beli ' + belanja[i] + ' melebihi stok yang tersedia ' + ' silakan beli dengan jumlah tidak lebih dari ' + item[0].jumlah);
                status = 0
            } else { }
        } else if (belanja[i] == item[1].nama) {
            if (jumlah[i] > item[1].jumlah) {
                $('#warning2').html('<span> Maaf. jumlah beli ' + belanja[i] + ' melebihi stok yang tersedia ' + ' silakan beli dengan jumlah tidak lebih dari ' + item[1].jumlah + '</span>');
                status = 0
            } else { }
        }
        else if (belanja[i] == item[2].nama) {
            if (jumlah[i] > item[2].jumlah) {
                $('#warning2').html('<span> Maaf. jumlah beli ' + belanja[i] + ' melebihi stok yang tersedia ' + ' silakan beli dengan jumlah tidak lebih dari ' + item[2].jumlah + '</span>');
                status = 0
            } else { }
        }
        else if (belanja[i] == item[3].nama) {
            if (jumlah[i] > item[3].jumlah) {
                $('#warning2').html('<span> Maaf. jumlah beli ' + belanja[i] + ' melebihi stok yang tersedia ' + ' silakan beli dengan jumlah tidak lebih dari ' + item[3].jumlah + '</span>');
                status = 0
            } else { }
        }
        else if (belanja[i] == item[4].nama) {
            if (jumlah[i] > item[4].jumlah) {
                $('#warning2').html('<span> Maaf. jumlah beli ' + belanja[i] + ' melebihi stok yang tersedia ' + ' silakan beli dengan jumlah tidak lebih dari ' + item[4].jumlah + '</span>');
                status = 0
            } else { }
        }
    };
